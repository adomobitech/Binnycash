...
export const DeviceIcon = ({ offer }: { offer: any }) => {
  ...
  const isIos = p.includes('ios') || p.includes('iphone') || p.includes('ipad');
  const isMac = p.includes('mac') || p.includes('osx');
  const isWindowsOrWeb = p.includes('windows') || p.includes('win') || p.includes('desktop') || p.includes('pc') || p.includes('web');

  // 1. Android
  if (isAndroid && !isIos) {
    return (
      <svg className="w-3.5 h-3.5 text-[#A4C639]" viewBox="0 0 24 24" fill="currentColor">
  ...
  // 2. Windows / Web (Pehle check hoga taaki web/windows offers par hamesha Windows icon hi aaye)
  if (isWindowsOrWeb && !isIos && !isMac) {
    return (
      <svg className="w-3.5 h-3.5 text-[#00A4EF]" viewBox="0 0 24 24" fill="currentColor">
