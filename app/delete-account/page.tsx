...
export default function DeleteAccountPage() {
  ...
  // Page States - STRICTLY TEMPORARY (No Local Storage)
  ...
  // 1. Handle Isolated Login Verification (NO LOCAL STORAGE SAVING)
  const handleVerifyIdentity = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoggingIn(true);
    setMessage(null);
    
    // Generate a temporary random device ID just for this API call
    ...
    try {
      ...
      if (!isError) {
        ...
        // 🔥 STRICT FIX: We ONLY save the token to this page's memory. It does NOT log them into the website. 🔥
  ...
  // 2. Handle Final Account Deletion
  const handleDeleteAccount = async () => {
    ...
    // Ensure we have the temporary token
    if (!tempToken) {
    ...
    try {
      ...
      if (!isError) {
        ...
        // Wipe local storage just in case they WERE logged in on the main site, then exit.
        setTimeout(() => {
          localStorage.clear();
  ...
  // 3. Handle Safe Exit (Complete Reset)
  const handleSafeExit = () => {
    ...
    setConfirmText('');
    setIsApproved(false);
    // Hard redirect to clear page memory
    window.location.href = '/';
  };
  ...
  return (
    <div className="min-h-screen bg-[#0B0D14] text-[#F5F3FF] selection:bg-[#FF5D73]/30 relative overflow-x-hidden font-sans py-12">
      ...
      {/* 🔥 CSS HACK: HIDE NAVBAR & TICKER SO THIS LOOKS LIKE A STANDALONE TOOL 🔥 */}
      ...
      <main className="max-w-[600px] mx-auto px-4 sm:px-6 relative z-10 mt-10">
        ...
        >
          <AnimatePresence mode="wait">
            ...
            {/* ======================================================== */}
            {/* STEP 1: VERIFY IDENTITY (NO BROWSER SAVING)              */}
            {/* ======================================================== */}
            {step === 'auth' && (
              <motion.div 
                key="auth"
                ...
                <div className="p-8 sm:p-10">
                  ...
                  <form onSubmit={handleVerifyIdentity} className="flex flex-col gap-4">
                    ...
                    <button 
                      ...
                      className="mt-4 w-full bg-gradient-to-r from-[#8B5CF6] to-[#6D28D9] text-white font-black text-sm uppercase tracking-widest py-4 rounded-[12px] shadow-[0_4px_15px_rgba(139,92,246,0.3)] hover:shadow-[0_6px_20px_rgba(139,92,246,0.4)] transition-all cursor-pointer flex items-center justify-center gap-2"
                    >
            ...
            {/* ======================================================== */}
            {/* STEP 2: CONFIRM DELETION (DANGER ZONE)                   */}
            {/* ======================================================== */}
            {step === 'confirm' && (
              <motion.div 
                key="confirm"
                ...
                <div className="p-8 sm:p-10">
                  <div className="bg-[#1A1C24] border border-white/5 rounded-2xl p-6 mb-8">
                    ...
                    <ul className="space-y-3">
                      <li className="flex items-start gap-3 text-[#8F95A3] text-sm">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#FF5D73] mt-1.5 shrink-0" />
                        Your profile, personal data, and connected accounts will be permanently wiped.
                      </li>
                      <li className="flex items-start gap-3 text-[#8F95A3] text-sm">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#FF5D73] mt-1.5 shrink-0" />
                        Any unredeemed balance, rewards, and pending cashouts will be forfeited.
                      </li>
                      <li className="flex items-start gap-3 text-[#8F95A3] text-sm">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#FF5D73] mt-1.5 shrink-0" />
                        Your referral network and lifetime earnings history will be lost.
                      </li>
                    </ul>
                  </div>
                  ...
                  {/* Input Confirmation */}
                  <div className="mb-5">
                    <label className="block text-sm font-bold text-[#8F95A3] mb-3">
                      To confirm, type <span className="text-white font-black bg-white/10 px-2 py-0.5 rounded">DELETE</span> below:
                    </label>
                    <input 
                      type="text" 
                      value={confirmText}
                      ...
                      className="w-full bg-[#0B0D14] border border-[#1A1D24] text-white font-bold rounded-xl px-5 py-4 outline-none focus:border-[#FF5D73] focus:bg-[#161217] transition-all placeholder:text-[#4B5263] shadow-inner"
                    />
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <button 
                      ...
                      className="flex-1 py-4 rounded-xl bg-white/5 hover:bg-white/10 text-white font-bold text-sm transition-colors cursor-pointer border border-white/5"
                    >
                      Cancel, keep my account
                    </button>
                    
                    <button 
                      disabled={isDeleting || !isApproved || confirmText !== 'DELETE'}
                      onClick={handleDeleteAccount}
                      className="flex-1 py-4 rounded-xl bg-gradient-to-r from-[#FF5D73] to-[#E11D48] text-white font-black text-sm transition-all shadow-[0_0_20px_rgba(255,93,115,0.3)] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2 hover:shadow-[0_0_30px_rgba(255,93,115,0.5)]"
                    >
